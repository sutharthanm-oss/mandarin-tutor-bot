...
// Includes loudness normalization (loudnorm) since the raw TTS output can come
// through quiet — this brings it up to a consistent, clearly audible level.
export function convertToOggOpus(inputPath) {
  return new Promise((resolve, reject) => {
    ...
    const ffmpeg = spawn('ffmpeg', [
      ...
      '-ar', '48000', '-ac', '1', '-c:a', 'libopus', '-b:a', '32k',
      outPath,
    ]);
    let stderr = '';
    ffmpeg.stderr.on('data', d => (stderr += d));
